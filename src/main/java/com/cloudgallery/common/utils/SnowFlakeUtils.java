...
public class SnowFlakeUtils {
    ...
    public SnowFlakeUtils(){
        ...
        if (this.workerId > this.maxWorkerId || this.workerId < 0) {
            throw new CgServiceException(String.format("worker Id can't be greater than %d or less than 0",maxWorkerId));
        }
        if (this.datacenterId > this.maxDatacenterId || this.datacenterId < 0) {
            throw new CgServiceException(String.format("datacenter Id can't be greater than %d or less than 0",maxDatacenterId));
        }
   ...
   //数据id需要左移位数 12+5=17位
    ...
    public static void main(String[] args) {
        SnowFlakeUtils worker = new SnowFlakeUtils();
        for (int i = 0; i < 30; i++) {
